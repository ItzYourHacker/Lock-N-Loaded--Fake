...
class Vanity(commands.Cog):
    ...
    def help_custom(self):
		      ...
		      label = "Vanity"
		      description = "Shows Vanity Roles Commands"
    ...
    async def on_presence_update(self, before, after):
      ...
      if str(before.guild.id) not in jnl:
        ...
        if vanity in str(after.activity).lower() and grole not in after.roles:
            ...
            embed.set_footer(text="Thanks For Choosing Lock N Loaded.")
            ...
            embed=discord.Embed(color=0x030404, description=f"{after.mention} Removed [Lock N Loaded Hq](https://discord.gg/lnlhq) Vanity {vanity} From His/Her Status <3".title(), timestamp=datetime.datetime.utcnow())
            embed.set_author(name=after.name, icon_url=after.display_avatar.url)
            embed.set_footer(text="Thanks For Choosing Lock N Loaded")
            await gchannel.send(embed=embed)
