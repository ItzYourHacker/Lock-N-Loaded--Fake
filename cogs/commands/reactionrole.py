...
class ReactionRoles(commands.Cog):
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if role is not None and user is not None:
            await user.add_roles(role, reason="Ventura | Reaction Role")

    @commands.Cog.listener()
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
        role, user = self.parse_reaction_payload(payload)
        if role is not None and user is not None:
            await user.remove_roles(role, reason="Ventura | Reaction Role")

    @commands.has_permissions(manage_channels=True)
    @commands.command(aliases=['rrcreate'])
    async def rr(self, ctx, emote, role: discord.Role, channel: discord.TextChannel, *, title=None):
        if title is None:
